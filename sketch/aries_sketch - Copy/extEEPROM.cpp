...
extEEPROMFast::extEEPROMFast(eeprom_size_t deviceCapacity, byte nDevice, unsigned int pageSize, uint8_t eepromAddr)
{
...
byte extEEPROMFast::begin(twiClockFreq_t twiFreq, TwoWire *_comm)
{
    communication = _comm;
    communication->begin();
...
byte extEEPROMFast::write(unsigned long addr, byte *values, unsigned int nBytes)
{
...
//a status of EEPROM_ADDR_ERR is returned. For I2C errors, the status
//from the Arduino Wire library is passed back through to the caller.
byte extEEPROMFast::read(unsigned long addr, byte *values, unsigned int nBytes)
{
    byte ctrlByte;
    byte rxStatus;
...
//a status of EEPROM_ADDR_ERR is returned. For I2C errors, the status
//from the Arduino Wire library is passed back through to the caller.
byte extEEPROMFast::write(unsigned long addr, byte value)
{
...
int extEEPROMFast::read(unsigned long addr)
{
    uint8_t data;
    int ret;
...
byte extEEPROMFast::update(unsigned long addr, byte *values, unsigned int nBytes)
{
    if (nBytes == 1) {
...
//For I2C errors, the status from the Arduino Wire library is passed back through to the caller.
byte extEEPROMFast::update(unsigned long addr, byte value)
{
    return (value != read(addr) ? write(addr, &value, 1) : 0);
}

//For I2C errors, the status from the Arduino Wire library is passed back through to the caller.
unsigned long extEEPROMFast::length( void )
{
